...
def return_data():
    ...
    for i in range(min(15, len(a_tags))):  # restricting to 15 articles
    ...
    for link in links:
        ...
        if (
            link.find("video") == -1
        ):  # Excluding news links with videos instead of articles
            ...
            if outer != []:
                ...
                for i in range(
                    1, len(paras) - 1
                ):  # Excluding  tags with irrelevant info. like date, time, author etc.
                    curr += paras[i].text
